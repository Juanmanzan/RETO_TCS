...
async def actualizar_saldos_transacciones_lote(
    sesion: AsyncSession,
    transacciones: Sequence[Transaccion],
) -> None:

    deltas_por_cuenta: dict[int, int] = defaultdict(int)
    ...
    for cuenta_id, delta_centavos in (
        deltas_por_cuenta.items()
    ):
...
async def insertar_transacciones_lote(
    ...
    try:
        ...
        await actualizar_saldos_transacciones_lote(
            sesion=sesion,
            transacciones=transacciones_insertadas,
        )
